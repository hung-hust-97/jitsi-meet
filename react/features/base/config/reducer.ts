...
ReducerRegistry.register<IConfigState>(
    "features/base/config",
    (state = _getInitialState(), action): IConfigState => {
        switch (action.type) {
            case UPDATE_CONFIG:
...
function _translateInterfaceConfig(oldValue: IConfig) {
    ...
    if (oldValue.defaultLogoUrl === undefined) {
        if (
            ...
            newValue.defaultLogoUrl = "images/logo.svg";
        }
    }
    ...
    if (oldValue.deeplinking && newValue.deeplinking) {
        // make TS happy
        newValue.deeplinking.disabled = oldValue.deeplinking.hasOwnProperty(
            "disabled"
        )
        ...
        if (typeof interfaceConfig === "object") {
            ...
            const dynamicLink: IMobileDynamicLink | undefined =
                mobileDynamicLink
                    ? {
            ...
            deeplinking.hideLogo = Boolean(
                interfaceConfig.HIDE_DEEP_LINKING_LOGO
            );
            deeplinking.android = {
...
function _translateLegacyConfig(oldValue: IConfig) {
    ...
    ).filter((key) => oldValue[key as keyof IConfig]);
        ...
        filteredConferenceInfo.forEach((key) => {
            ...
            if (key === "hideRecordingLabel") {
                ...
                newValue.conferenceInfo.alwaysVisible = (
                    ...
                    (c) =>
                        !CONFERENCE_HEADER_MAPPING[
                            key as keyof typeof CONFERENCE_HEADER_MAPPING
                        ].includes(c)
                );
                newValue.conferenceInfo.autoHide = (
                    newValue.conferenceInfo.autoHide ?? []
    ...
    if (
        oldValue.hasOwnProperty("enableWelcomePage") &&
    ...
    if (
        oldValue.fileRecordingsServiceEnabled !== undefined &&
    ...
    if (
        oldValue.disableSpeakerStatsSearch !== undefined &&
    ...
    if (
        oldValue.speakerStatsOrder !== undefined &&
    ...
    if (
        oldValue.autoKnockLobby !== undefined &&
    ...
    if (
        oldValue.enableLobbyChat !== undefined &&
    ...
    if (
        oldValue.hideLobbyButton !== undefined &&
