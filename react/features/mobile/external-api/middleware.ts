...
externalAPIEnabled &&
    MiddlewareRegistry.register((store) => (next) => (action) => {
        const oldAudioMuted =
            store.getState()["features/base/media"].audio.muted;
        ...
        switch (type) {
            ...
            case PARTICIPANT_LEFT: {
                ...
                const isVirtualScreenshareParticipant =
                    isScreenShareParticipantById(
                        store.getState(),
                        participant.id
                    );
...
externalAPIEnabled &&
    StateListenerRegistry.register(
        ...
        /* listener */ debounce((tracks: ITrack[], store: IStore) => {
            const oldScreenShares =
                store.getState()["features/mobile/external-api"].screenShares ||
                [];
            const newScreenShares = tracks
                .filter(
                    (track) =>
                        track.mediaType === MEDIA_TYPE.SCREENSHARE ||
...
function _registerForNativeEvents(store: IStore) {
    ...
    eventEmitter.addListener(
        ExternalAPI.SEND_ENDPOINT_TEXT_MESSAGE,
        ({ to, message }: any) => {
    ...
    eventEmitter.addListener(
        ExternalAPI.RETRIEVE_PARTICIPANTS_INFO,
        ({ requestId }: any) => {
            ...
            remoteParticipants.forEach((participant) => {
                if (!participant.fakeParticipant) {
                    participantsInfo.push(
                        participantToParticipantInfo(participant)
                    );
                }
            });
 ...
 * @returns {void}
 */
function _maybeTriggerEarlyConferenceWillJoin(
    store: IStore,
    action: AnyAction
) {
 ...
 */
function _swallowConferenceLeft(
    { getState }: IStore,
    action: AnyAction,
    { url }: { url: string }
) {
